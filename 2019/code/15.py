...
with open("2019/inputs/15.txt") as file:
...
show = True  # False for faster testing.
if show:
    pygame.init()
...
while running:
    ...
    if not fully_explored:
        new_position = tuple_add(position, offsets[directions_i])
        if new_position == origin:
            fully_explored = True
            walkable = {k for k, v in world.items() if v == "walkable"}

        droid.add_input(directions[directions_i])
        droid.compute()
        status = droid.pop_output()
    ...
    if show:
        ...
        pygame.display.flip()
        dt.tick(frames_per_second)

# part 1:
print('Part 1 Answer: {}'.format(len(path)))

# part 2:
print('Part 2 Answer: {}'.format(oxygen_minutes))
