...
def get_bots(values):
    r = re.compile(r'pos=<([0-9-]+),([0-9-]+),([0-9-]+)>, r=([0-9]+)')
    bots = []
    for cur in values:
        if cur.startswith("#"):
            print("# Note: " + cur)
...
def run(values):
    print("Part 1 Answer:", (calc(values)))
    print("Part 2 Answer:", (calc2(values)))

vals = open('2018/inputs/2018_23_input.txt', 'r').readlines()
run(vals)